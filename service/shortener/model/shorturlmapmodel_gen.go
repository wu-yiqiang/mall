...
type (
	...
	ShortUrlMap struct {
		...
		Create_By string         `db:"create by"` // 创建者
...
func (m *defaultShortUrlMapModel) Insert(ctx context.Context, data *ShortUrlMap) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Create_By, data.IsDel, data.Lurl, data.Md5, data.Surl)
	return ret, err
}
...
func (m *defaultShortUrlMapModel) Update(ctx context.Context, newData *ShortUrlMap) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Create_By, newData.IsDel, newData.Lurl, newData.Md5, newData.Surl, newData.Id)
	return err
}
